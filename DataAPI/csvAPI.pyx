# cython: language_level=3
import os
from libc.stdlib cimport malloc, free

from Common.CEnum cimport DATA_FIELD, KL_TYPE
from Common.ChanException cimport CChanException, ErrCode
from Common.CTime cimport CTime
from Common.func_util cimport str2float
from KLine.KLine_Unit cimport CKLine_Unit

from .CommonStockAPI cimport CCommonStockApi

cdef dict create_item_dict(list data, list column_name):
    cdef:
        int i
        dict result = {}
    for i in range(len(data)):
        data[i] = parse_time_column(data[i]) if column_name[i] == DATA_FIELD.FIELD_TIME else str2float(data[i])
    for i in range(len(column_name)):
        result[column_name[i]] = data[i]
    return result

cdef CTime parse_time_column(str inp):
    cdef:
        int year, month, day, hour, minute
    if len(inp) == 10:
        year = int(inp[:4])
        month = int(inp[5:7])
        day = int(inp[8:10])
        hour = minute = 0
    elif len(inp) == 17:
        year = int(inp[:4])
        month = int(inp[4:6])
        day = int(inp[6:8])
        hour = int(inp[8:10])
        minute = int(inp[10:12])
    elif len(inp) == 19:
        year = int(inp[:4])
        month = int(inp[5:7])
        day = int(inp[8:10])
        hour = int(inp[11:13])
        minute = int(inp[14:16])
    else:
        raise ValueError(f"unknown time column from csv:{inp}")
    return CTime(year, month, day, hour, minute)

cdef class CSV_API(CCommonStockApi):
    cdef:
        public bint headers_exist
        public list columns
        public int time_column_idx

    def __cinit__(self, str code, object k_type=KL_TYPE.K_DAY, object begin_date=None, object end_date=None, object autype=None):
        super().__init__(code, k_type, begin_date, end_date, autype)
        self.headers_exist = True  # 第一行是否是标题，如果是数据，设置为False
        self.columns = [
            DATA_FIELD.FIELD_TIME,
            DATA_FIELD.FIELD_OPEN,
            DATA_FIELD.FIELD_HIGH,
            DATA_FIELD.FIELD_LOW,
            DATA_FIELD.FIELD_CLOSE,
            DATA_FIELD.FIELD_VOLUME,
            # DATA_FIELD.FIELD_TURNOVER,
            # DATA_FIELD.FIELD_TURNRATE,
        ]  # 每一列字段
        self.time_column_idx = self.columns.index(DATA_FIELD.FIELD_TIME)

    cpdef get_kl_data(self):
        cdef:
            str cur_path = os.path.dirname(os.path.realpath(__file__))
            str file_path = f"{cur_path}/{self.code}.csv"
            int line_number
            str line
            list data
            dict item_dict

        if not os.path.exists(file_path):
            raise CChanException(f"file not exist: {file_path}", ErrCode.SRC_DATA_NOT_FOUND)

        with open(file_path, 'r') as f:
            for line_number, line in enumerate(f):
                if self.headers_exist and line_number == 0:
                    continue
                data = line.strip("\n").split(",")
                if len(data) != len(self.columns):
                    raise CChanException(f"file format error: {file_path}", ErrCode.SRC_DATA_FORMAT_ERROR)
                if self.begin_date is not None and data[self.time_column_idx] < self.begin_date:
                    continue
                if self.end_date is not None and data[self.time_column_idx] > self.end_date:
                    continue
                item_dict = create_item_dict(data, self.columns)
                yield CKLine_Unit(item_dict)

    cpdef SetBasciInfo(self):
        pass

    @classmethod
    def do_init(cls):
        pass

    @classmethod
    def do_close(cls):
        pass

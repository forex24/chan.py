...
from Bi.BiConfig cimport CBiConfig
from BuySellPoint.BSPointConfig cimport CBSPointConfig
from Common.CEnum cimport TREND_TYPE
from Common.ChanException cimport CChanException, ErrCode
from Common.func_util cimport _parse_inf
from Math.BOLL cimport BollModel
from Math.Demark cimport CDemarkEngine
from Math.KDJ cimport KDJ
from Math.MACD cimport CMACD
from Math.RSI cimport RSI
from Math.TrendModel cimport CTrendModel
from Seg.SegConfig cimport CSegConfig
from ZS.ZSConfig cimport CZSConfig

cdef class CChanConfig:
    cdef:
        public CBiConfig bi_conf
        public CSegConfig seg_conf
        public CZSConfig zs_conf
        public bint trigger_step
        public int skip_step
        public bint kl_data_check
        public int max_kl_misalgin_cnt
        public int max_kl_inconsistent_cnt
        public bint auto_skip_illegal_sub_lv
        public bint print_warning
        public bint print_err_time
        public list mean_metrics
        public list trend_metrics
        public dict macd_config
        public bint cal_demark
        public bint cal_rsi
        public bint cal_kdj
        public int rsi_cycle
        public int kdj_cycle
        public dict demark_config
        public int boll_n
        public CBSPointConfig bs_point_conf
        public CBSPointConfig seg_bs_point_conf

    def __cinit__(self, dict conf=None):
        if conf is None:
            conf = {}
    ...
    cpdef list GetMetricModel(self):
        cdef:
            list res = [
                CMACD(
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cpdef void set_bsp_config(self, dict conf):
        cdef:
            dict para_dict = {
...
cdef class ConfigWithCheck:
    cdef:
        public dict conf

    def __cinit__(self, dict conf):
        self.conf = conf

    cpdef object get(self, str k, object default_value=None):
        cdef object res = self.conf.get(k, default_value)
        if k in self.conf:
            del self.conf[k]
        return res

...
impl CTrendLine {
    fn new(lst: Vec<BI>, side: TREND_LINE_SIDE) -> Self {
        let mut trend_line = Self { line: None, side };
        trend_line.cal(lst);
        trend_line
    }

    fn cal(&mut self, lst: Vec<BI>) {
...
fn init_peak_slope(_dir: BI_DIR, side: TREND_LINE_SIDE) -> f64 {
    match side {
...
fn cal_tl(c_p: &[Point], _dir: BI_DIR, side: TREND_LINE_SIDE) -> (Line, usize) {
    let p = c_p[0].clone();
...
impl BI {
    fn get_begin_klu(&self) -> &Point {
        &self.begin_klu
    }

    fn get_end_klu(&self) -> &Point {
        &self.end_klu
    }
